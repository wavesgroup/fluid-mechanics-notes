...
ϕ_ = ϕ(a, ω(g, k, h), k, ζ, ψ_, g)
u_ = u(a, ω(g, k, h), k, ζ, ψ_)
w_ = w(a, ω(g, k, h), k, ζ, ψ_)


fig, ax = plt.subplots(figsize=(16, 4))
im = ax.contourf(ψ_, ζ - z.max(), ϕ_, np.arange(-0.5, 0.55, 0.05), cmap=cm.bwr)
...
plt.tight_layout()
plt.savefig("fig_wave_potential.png", dpi=150)
plt.close()


fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(16, 8))

im1 = ax1.contourf(ψ_, ζ - z.max(), u_, np.arange(-0.5, 0.55, 0.05), cmap=cm.bwr)
plt.colorbar(im1, ax=ax1)
ax1.plot(ψ, η(a, ψ), "k-")
ax1.plot(ψ, 0 * ψ, "k--")
ax1.set_title("Horizontal velocity (m/s), $a = 0.1$ m, $k = 1$ rad m$^{-1}$")

im2 = ax2.contourf(ψ_, ζ - z.max(), w_, np.arange(-0.5, 0.55, 0.05), cmap=cm.bwr)
...
plt.tight_layout()
plt.savefig("fig_wave_velocities.png", dpi=150)
plt.close()
